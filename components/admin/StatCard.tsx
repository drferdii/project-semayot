...
export function StatCard({ label, value, valueCents, accent = 'neutral', delta, sparkline, className }: StatCardProps) {
  const colorMap = {
  ...
  return (
    ...
    >
      <div>
        ...
        <div className={cn('font-display text-2xl font-semibold tracking-tight tabular-nums', colorMap[accent])}>
        ...
        <div className="mt-5">
          {delta && (
            <div className={cn(
              ...
              accent === 'success' || accent === 'neutral' ? 'text-emerald-700 bg-emerald-500/5 border-emerald-600/20' : 'text-red-700 bg-red-600/5 border-red-600/20'
            )}>
              {delta}
            </div>
          )}
          {sparkline && (
            <div className="mt-4 h-6 opacity-30">
              <svg className="w-full h-full" viewBox="0 0 100 24" preserveAspectRatio="none">
                <polyline points={sparkline} fill="none" stroke="var(--foreground)" strokeWidth="1.5" strokeLinejoin="round" strokeLinecap="round" />
                <polygon points={`${sparkline} 100,24 0,24`} fill="var(--foreground)" opacity="0.05" />
              </svg>
            </div>
          )}
        </div>
